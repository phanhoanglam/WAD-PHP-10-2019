...
if (isset($_GET['edit'])) {
    $update = true;
    $id = $_GET['edit'];
    $sql = "SELECT products.id, products.Name, product_categories.name as CategoryName, product_categories.id as Categoryid, 
        description, price, image, created, discount FROM products
        JOIN product_categories 
        ON products.product_category_id = product_categories.id WHERE products.id = $id";
    $record = mysqli_query($connect, $sql);
    if ($record) {
        $n = mysqli_fetch_array($record);
        $name = $n['Name'];
        $productid = $n['Categoryid'];
        $description = $n['description'];
        $price = $n['price'];
        $discount = $n['discount'];
        $imageProduct = $n['image'];
    }
}
...
if (isset($_POST['updateProduct'])) {
    ...
    if ($image_file != null) {
        ...
        $sql = "Update products info SET Name='$name', product_category_id='$productCategory', description='$description'
        , image='$target_file', price='$price', discount='$discount', updated='$update' WHERE id=$id";
    ...
    $sql = "Update products info SET Name='$name', product_category_id='$productCategory', description='$description'
    , price='$price', discount='$discount', updated='$update' WHERE id=$id";
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="p-5">
                            ...
                            <form class="user" action="EditProduct.php" method="POST" enctype='multipart/form-data'>
                                <input type="hidden" name="id" value="<?php echo $id; ?>">
                                <div class="form-group">
                                    <input type="text" value="<?php echo $name; ?>" class="form-control form-control-user" name="Name" placeholder="Product Name">
                                </div>
                                <div class="form-group">
                                    <select class="form-control" name="Category">
                                        <?php while ($rows = mysqli_fetch_array($category)) { ?>
                                            <?php if ($productid == $rows['id']) { ?>
                                ...
                                <div class="form-group">
                                    <input type="text" value="<?php echo $description; ?>" class="form-control form-control-user" name="Description" placeholder="Description">
                                </div>
                                <div class="form-group">
                                    ...
                                    <div>
                                        <img src="<?php echo $imageProduct; ?>" width="20%">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <input type="text" value="<?php echo $price; ?>" class="form-control form-control-user" name="Price" placeholder="Price">
                                </div>
                                <div class="form-group">
                                    <input type="text" value="<?php echo $discount; ?>" class="form-control form-control-user" name="Discount" placeholder="Discount">
                                </div>
